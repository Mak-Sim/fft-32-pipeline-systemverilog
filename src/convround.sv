...
module	convround #(
		...
		input	logic	signed	[(IWID-1):0]	i_val,
		output	logic	signed	[(OWID-1):0]	o_val
	);
	...
	begin : DROP_ONE_BIT // Output drops one bit, can only add one or ... not.
		logic	[(OWID-1):0]	truncated_value, rounded_up;
		logic			last_valid_bit, first_lost_bit;
	...
	begin : ROUND_RESULT
		...
		logic	[(IWID-SHIFT-OWID-2):0]	other_lost_bits;
