...
module	qtrstage(i_clk, i_reset, i_ce, i_sync, i_data, o_data, o_sync);
	...
	logic	signed [(IWIDTH):0]	sum_r, sum_i, diff_r, diff_i;

	logic	[(2*OWIDTH-1):0]	ob_a;
	logic	[(2*OWIDTH-1):0]	ob_b;
	logic	[(OWIDTH-1):0]		ob_b_r, ob_b_i;
	assign	ob_b = { ob_b_r, ob_b_i };

	logic	[(LGWIDTH-1):0]		iaddr;
	logic	[(2*IWIDTH-1):0]	imem	[0:1];

	logic	signed	[(IWIDTH-1):0]	imem_r, imem_i;
	assign	imem_r = imem[1][(2*IWIDTH-1):(IWIDTH)];
	assign	imem_i = imem[1][(IWIDTH-1):0];

	logic	signed	[(IWIDTH-1):0]	i_data_r, i_data_i;
	assign	i_data_r = i_data[(2*IWIDTH-1):(IWIDTH)];
	assign	i_data_i = i_data[(IWIDTH-1):0];

	logic	[(2*OWIDTH-1):0]	omem [0:1];

	//
	// Round our output values down to OWIDTH bits
	//
	logic	signed	[(OWIDTH-1):0]	rnd_sum_r, rnd_sum_i,
